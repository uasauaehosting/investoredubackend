/**
 * Seeds all website content from frontend static files into MySQL.
 * Run: npx ts-node src/scripts/seed-all-content.ts
 */
import 'dotenv/config';
import fs from 'fs';
import path from 'path';
import { executeQuery, executeInsert, initConnection } from '../utils/database';
import { SiteContent, AboutSection, FooterStats } from '../models';
import { GlossaryTerm } from '../models/Glossary';
import { PortalModel } from '../models/Portals';
import { InvestmentProductModel } from '../models/InvestorEducation';
...
import { PROGRAMS_SEED_DATA } from '../data/programsSeed';

const FRONTEND = path.resolve(__dirname, '../../../investoredufrontend/src');
...
async function runMigrations() {
  ...
  try {
    await executeQuery('ALTER TABLE investment_products ADD COLUMN slug VARCHAR(255) DEFAULT NULL');
  } catch {
    // column may already exist
  }

  await executeQuery(`CREATE TABLE IF NOT EXISTS glossary_terms (
    id INT AUTO_INCREMENT PRIMARY KEY,
...
async function seedSiteContent() {
  ...
  for (const [key, content] of Object.entries(blocks)) {
    await SiteContent.upsert(key, content);
    console.log(`  ✓ site_content: ${key}`);
  }
}

async function seedAboutSections() {
  const existing = await AboutSection.findAll();
...
async function seedGlossary() {
  const glossaryPath = path.join(FRONTEND, 'data/glossaryTerms.json');
  if (!fs.existsSync(glossaryPath)) {
    console.log('  ⊘ glossary JSON not found');
    return;
  }

  const existing = await GlossaryTerm.findAll();
  ...
  const terms: Array<{ english: string; french: string; arabic: string; definition: string }> = data.terms;
  ...
  for (const term of terms) {
    ...
    count++;
    if (count % 200 === 0) console.log(`    ... ${count} glossary terms`);
  }
  console.log(`  ✓ glossary: ${count} terms`);
}

async function seedMemberPortals() {
  const existing = await PortalModel.getAll({ is_active: true });
  if (existing.length >= 5) {
    console.log(`  ⊘ portals already has ${existing.length} entries`);
    return;
  }

  const portalsPath = path.join(FRONTEND, 'data/memberPortals.ts');
...
async function seedInvestmentProducts() {
  const existing = await InvestmentProductModel.findAll();
  if (existing.length >= 3) {
    console.log(`  ⊘ investment_products already has ${existing.length} entries`);
    return;
  }

  // Load products from frontend static data via require (ts-node resolves .ts)
  // eslint-disable-next-line @typescript-eslint/no-var-requires
  const productsModule = require(path.join(FRONTEND, 'lib/investmentProducts'));
  const INVESTMENT_PRODUCTS: Array<{ slug: string; title: string; imageUrl: string; excerpt: string; blocks: unknown[] }> =
    productsModule.INVESTMENT_PRODUCTS ?? [];

  for (const product of INVESTMENT_PRODUCTS) {
    await executeInsert(
      `INSERT INTO investment_products (title, description, author, date, file_url, image_url, content, slug, views, downloads, is_active)
       VALUES (?, ?, '', CURDATE(), '', ?, ?, ?, 0, 0, true)`,
      [product.title, product.excerpt, product.imageUrl, JSON.stringify({ blocks: product.blocks }), product.slug],
    );
  }
...
async function seedAdmins() {
  const admins = await executeQuery<{ username: string }>('SELECT username FROM admins WHERE is_active = true');
  const usernames = new Set(admins.map((a) => a.username));

  const defaults = [
    { username: 'superadmin', email: 'superadmin@uasa.ae', password: 'admin123', firstName: 'Super', lastName: 'Admin', role: 'Super Admin' as const, permissions: ['all'] },
    { username: 'admin', email: 'admin@uasa.ae', password: 'admin123', firstName: 'System', lastName: 'Admin', role: 'Admin' as const, permissions: ['content'] },
  ];
  ...
  for (const admin of defaults) {
    ...
    await AdminModel.create({ ...admin, isActive: true });
...
async function seedPublications() {
  ...
  console.log(`  ✓ publications: ${PUBLICATIONS_SEED_DATA.length} entries`);
}

async function seedPrograms() {
  const existing = await ProgramsModel.count();
  if (existing >= PROGRAMS_SEED_DATA.length) {
  ...
  for (const row of PROGRAMS_SEED_DATA) {
  ...
  console.log(`  ✓ programs: ${PROGRAMS_SEED_DATA.length} entries`);
}

async function seedEducationContent() {
  const existing = await EducationContentModel.findAllAdmin();
  ...
  const alerts = await executeQuery<{ title: string; description: string; type: string }>(
    'SELECT title, description, type FROM alerts_bulletins WHERE is_active = true ORDER BY date_published DESC LIMIT 6',
  );
  ...
  for (const card of cards) {
    await EducationContentModel.create({
