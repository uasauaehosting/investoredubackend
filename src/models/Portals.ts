...
export class PortalModel {
    // Get all portals with optional filters
    static async getAll(filters: PortalFilters = {}): Promise<Portal[]> {
        let query = `
            SELECT * FROM portals 
            WHERE 1=1
        `;
        ...
        const results = await executeQuery<Portal>(query, params);
        return results;
    }

    // Get portal by ID
    static async getById(id: number): Promise<Portal | null> {
        const query = 'SELECT * FROM portals WHERE id = ?';
        const result = await executeSingleQuery<Portal>(query, [id]);
        return result;
    }

    // Create new portal
    static async create(data: CreatePortalData): Promise<Portal> {
        ...
        const query = `
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)
        `;
        const insertId = await executeInsert(query, [title, titleAr, short_title, short_titleAr, description, descriptionAr, image_url, link, authority_name, authority_nameAr, country, countryAr, display_order]);
        return await this.getById(insertId);
    }

    // Update portal
    static async update(id: number, data: UpdatePortalData): Promise<Portal | null> {
        const fields: string[] = [];
        const values: any[] = [];
    ...
    private static async getMockData(): Promise<Portal[]> {
        return [
            {
                id: 1,
