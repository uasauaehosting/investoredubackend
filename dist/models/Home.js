...
class NewsModel {
    static async create(newsData) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;
        return await (0, database_1.executeInsert)(query, [title, titleAr, excerpt, excerptAr, excerpt, excerptAr, fullDetail, fullDetailAr, category || 'News', link, image, pdfFile, newsDate, isActive]);
    }
...
class SlideModel {
    static async create(slideData) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `;
        return await (0, database_1.executeInsert)(query, [title, subtitle, image_url, cta_text, cta_href, display_order, is_active]);
    }
    static async findAll() {
