...
async function main() {
    ...
    const admin = await Admin_1.AdminModel.findByUsername('admin');
    if (admin) {
        const valid = await Admin_1.AdminModel.comparePassword('admin123', admin.password);
        checks.push({ name: 'admin login (admin/admin123)', ok: valid, detail: valid ? 'password valid' : 'password mismatch' });
        if (process.env.JWT_SECRET) {
            const token = jsonwebtoken_1.default.sign({ id: admin.id, username: admin.username }, process.env.JWT_SECRET, { expiresIn: '1h' });
...
//# sourceMappingURL=verify-api-data.js.map